...
% classifier training. Make sure the outputs are 'w' and 'b'.
features_total = [features_pos; features_neg];
labels = [ones(size(features_pos,1),1); -ones(size(features_neg,1),1)];
[w,b] = svm_classify(features_total, labels);
...
[sort_face_confs, sort_face_idx] = sort(face_confs);
[sort_non_face_confs, sort_non_face_idx] = sort(non_face_confs);
figure, 
plot(sort_face_confs, 'g'); hold on
plot(sort_non_face_confs,'r'); 
plot([0 size(non_face_confs,1)], [0 0], 'b');
hold off;

%% hard positive mining
hard_pos_idx = sort_face_idx(sort_face_confs<=1);
hard_pos_features = get_hard_positive(train_path_pos, feature_params, hard_pos_idx);

rep_n_hp = floor(size(features_pos,1)/size(hard_pos_features,1))-1;

%% hard negative mining
hard_neg_idx = sort_non_face_idx(sort_non_face_confs>=-1);
hard_neg_features = get_hard_negative(neg_examples, feature_params, hard_neg_idx);

rep_n_hn = floor(size(features_neg,1)/size(hard_neg_features,1)/4);

clear neg_examples

%% training with hard examples
features_pos_ap = [repmat(hard_pos_features,rep_n_hp,1); features_pos;];
features_neg_ap = [repmat(hard_neg_features,rep_n_hn,1); features_neg;];
assert(size(features_neg_ap,1)>size(features_pos_ap,1));
tmp = floor((size(features_neg_ap,1)-size(features_pos_ap,1))/size(features_pos_ap,1));
features_total = [repmat(features_pos_ap,tmp,1); features_neg_ap;];
labels = [ones(tmp*size(features_pos_ap,1),1); -ones(size(features_neg_ap,1),1)];
