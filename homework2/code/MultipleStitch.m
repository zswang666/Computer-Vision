...
for idx = refIdx + 1 : length(imageToRefTrans),  
    imageToRefTrans{idx} = makeTransformToReferenceFrame(TRANS, idx, refIdx);
    T = imageToRefTrans{idx};
%     T(3, :) = [0, 0, 1]; % Fix rounding errors in the last row. Extra constraints for affine transformation
    tmpBounds = findbounds(maketform('projective', T'), [1 1; ncols nrows]);
    outBounds(1,:) = min(outBounds(1,:),tmpBounds(1,:));
    outBounds(2,:) = max(outBounds(2,:),tmpBounds(2,:));
end
...
Pano = imtransform( im2double(IMAGES{refIdx}), maketform('projective', eye(3)), 'bilinear', ...
...
if currentFrameIndex < refFrameIndex,
    ...
    j = currentFrameIndex + 1;
    while( j<refFrameIndex ),
        T = i_To_iPlusOne_Transform{j} * T;
        j = j + 1;
    end
elseif currentFrameIndex > refFrameIndex,
    T = i_To_iPlusOne_Transform{refFrameIndex};
    j = refFrameIndex + 1;
    while( j<currentFrameIndex ),
        T = i_To_iPlusOne_Transform{j} * T;
        j = j + 1;
    end
    T = pinv(T);
