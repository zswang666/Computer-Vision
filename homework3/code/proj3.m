...
% FEATURE = 'tiny image';
FEATURE = 'bag of sift';
% FEATURE = 'spatial pyramid sifts';
% FEATURE = 'phow fisher vectors';
% FEATURE = 'fisher vector';
% FEATURE = 'placeholder';
...
%         save('sp_sifts.mat','train_image_feats','test_image_feats');
...
%         save('bags_of_sifts.mat','train_image_feats','test_image_feats');
        
    case 'phow fisher vectors'
        if ~exist('phow_fisher_vocab.mat', 'file')
            fprintf('No existing visual word vocabulary found. Computing one from training images\n')
            n_gmm = 30;
            vocab = build_phow_fisher_vocabulary(train_image_paths, n_gmm);
            num_training_samples = 500;
            PHOW_step = 10;
            save('phow_fisher_vocab.mat', 'vocab','n_gmm','num_training_samples','PHOW_step');
        end
        
        vocab_path = 'phow_fisher_vocab.mat';
        fprintf('train_image_feats, ');
        tic
...
%         PHOW_step = 5; 
%         save('phow_fisher_vectors.mat','train_image_feats','test_image_feats','PHOW_step');
    ...
    case 'fisher vector'
        if ~exist('fisher_vocab.mat', 'file')
            ...
            save('fisher_vocab.mat', 'vocab', 'n_gmm');
        end
        
        vocab_path = 'fisher_vocab.mat';
        fprintf('train_image_feats, ');
        tic
...
%         save('fisher_vectors.mat','train_image_feats','test_image_feats');
...
% load sp_sifts % debug
% CLASSIFIER = 'kernel svm'; % debug
% CLASSIFIER = 'pyramid kernel svm';  % debug
...
switch lower(CLASSIFIER)    
    ...
    case 'kernel svm'
        svm_conf = '-t 0 -e 1e-5 -c 0.01'; % linear kernel
%         svm_conf = '-t 1 -d 4 -g 1 -e 1e-5 -c 0.01'; % polynomial kernel
%         svm_conf = '-t 2 -g 0.001 -e 1e-5 -c 0.01'; % rbf kernel
