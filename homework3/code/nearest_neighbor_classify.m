...
for i=1:M,
    % test_i distance to all train_image_feats, 'Metric'={L0,L1,L2,LINF,CH12,HELL}
    i_to_all_dist = vl_alldist2(test_image_feats(i,:).',train_image_feats.','L2');
    % obtain minimum distance (one nearest neighbor)
    [~,min_idx] = min(i_to_all_dist);
    % assign label of first nearest neighbor to prediction
    predicted_categories{i} = train_labels{min_idx};
end
