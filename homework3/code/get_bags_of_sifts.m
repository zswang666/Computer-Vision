...
function image_feats = get_bags_of_sifts(image_paths,vocab_path)
...
textprogressbar('calculating bag of SIFTs: ');
...
for i=1:N,
    ...
    % form a histogram(VQ) feature vector of the image
    for j=1:size(SIFT_features,2),
        j_to_all_dist = vl_alldist2(single(SIFT_features(:,j)),vocab,'L2');
        [~,min_idx] = min(j_to_all_dist);
        image_feats(i,min_idx) = image_feats(i,min_idx) + 1;
    end
